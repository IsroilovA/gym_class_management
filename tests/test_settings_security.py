...
def test_production_enables_secure_flags():
    python_code = (
        ...
        'print(f"{settings.SECURE_SSL_REDIRECT},{settings.SESSION_COOKIE_SECURE},'
...
def test_non_production_defaults_are_developer_friendly():
    python_code = (
        'import django; import os; os.environ["DJANGO_SETTINGS_MODULE"]="config.settings"; django.setup(); '
        'from django.conf import settings; '
        'print(f"{settings.DEBUG},{settings.SECURE_SSL_REDIRECT},{settings.SESSION_COOKIE_SECURE},'
        '{settings.CSRF_COOKIE_SECURE},{settings.SECURE_HSTS_SECONDS}")'
    )
